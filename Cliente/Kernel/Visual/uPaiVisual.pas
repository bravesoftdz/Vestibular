...
type
  ...
  protected
    FControle: TControle;
    procedure CriarControlador(var poControle: TPersistent;
      poClasse: TPersistentClass);
...
procedure TPaiFrm.CriarControlador(var poControle: TPersistent; poClasse: TPersistentClass);
begin
  poControle := TComponentClass(poClasse).Create(Self);

  TPersistent(FControle) := (poControle as poClasse);
end;
...
begin
  if not Assigned(FControle) then
    raise Exception.Create('Desenvolvedor, atribuir a variável FControle ao criar o formulário herdado' + TForm(Sender).Name);
